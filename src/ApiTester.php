...
//        ApiLogger::log(...func_get_args());
    ...
    public function getRoutes()
    {
        ...
        $prefix = ApiTesterServiceProvider::setting('prefix', 'api');
    ...
    protected function getRouteParameters($action)
    {
        ...
        // foreach($classReflector->getMethod($method)->getParameters() as $parameter) {
        //     $clazz = $parameter->getClass();
        //     $func  = $clazz->getMethod("rules");
        //     dump($clazz, $func, $func->invoke($clazz->newInstance()), $clazz->newInstance() instanceof \Illuminate\Foundation\Http\FormRequest);
        // }
        ...
        if ($comment) {
            $parameters = [];
            preg_match_all('/\@[SWG|OA]+\\\Parameter\(\n*(.*?)\),?/is', $comment, $matches);
